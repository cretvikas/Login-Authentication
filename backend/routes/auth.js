...
const { validateUserLogin } = require('../utils/ipValidator');
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    // ── IP Validation for non-end_user roles ───────────────────
    const userRoles = user.roles ?? [];
    const ipValidation = validateUserLogin(userRoles, req.ip, ALLOWED_IP_RANGES);
    ...
    if (!ipValidation.allowed) {
      await writeAuditLog({
        ...
        ipAddress:  req.ip,
        userAgent:  req.headers['user-agent'],
        metadata:   { reason: 'IP address not whitelisted', roles: userRoles },
      });
      return res.status(403).json({ 
        error: 'Access denied: Your IP address is not whitelisted for this role.',
